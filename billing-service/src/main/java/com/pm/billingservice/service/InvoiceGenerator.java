...
public class InvoiceGenerator {
    ...
    public void generateInvoice(UUID billingAccountId) {
        ...
        // only 1 change is allowed during billing period
        ...
        log.info("Billing account {} has allowed Change: {} during billing period {} to {}.",
                billingAccountId, allowedChange != null ? allowedChange.getChangeType().name() : "NONE",
        ...
        // generate Invoice
        List<InvoiceLine> invoiceLines = Arrays.asList(baseLine, discountLine, prorationLine);
        Invoice invoice = Invoice.create(account, billingStartDate, billingEndDate,
                baseAmountCents, prorationLine.getAmountCents(), appliedDiscountCents, baseAmountCents + prorationLine.getAmountCents() + appliedDiscountCents, LocalDate.now(),
